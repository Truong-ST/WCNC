...
public class Factory {
    ...
    public double fitnessT(ArrayList<Integer> path, ArrayList<Double> timeCharge, Map map) {
        int N = map.getN(); // so luong sensor
        int death = 0;
        double distance = map.distanceCalculate(Factor.BS_INDEX, path.get(0));
        double waitingTime = 0;
        for (int i = 0; i < N; i++) {
            int sensor1 = path.get(i);
            int sensor2 = 0;
            ...
            waitingTime += map.distanceCalculate(sensor1, sensor2) / WCE.V;
            distance += map.distanceCalculate(sensor1, sensor2);
            if (waitingTime > (map.getSensor(sensor1).getE() - map.getSensor(sensor1).E_MIN)/map.getSensor(sensor1).getP()){
                death++;
//                System.out.println("===== wait time  : "+ waitingTime );
            }
//            System.out.println("===== E : "+map.getSensor(i).getE() );
//            System.out.println("===== Emin : "+map.getSensor(i).E_MIN );
//            System.out.println("===== P : "+ map.getSensor(i).getP() );
            //con thieu chet o cuoi chu trinh
            waitingTime += timeCharge.get(sensor1);
        }
